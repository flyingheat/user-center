...
import com.jingrui.usercenter.common.DeleteRequest;
...
import com.jingrui.usercenter.model.domain.request.*;
...
public class TeamController {
    ...
    public BaseResponse<List<TeamUserVO>> listTeams(TeamQuery teamQuery,HttpServletRequest request){
        ...
        //队伍 id=》加入这个队伍的用户列表
        Map<Long, List<UserTeam>> teamIdUserTeamId = userTeamList.stream().collect(Collectors.groupingBy(UserTeam::getTeamId));
        teamList.forEach(team -> {
            team.setHasJoinNum(teamIdUserTeamId.getOrDefault(team.getId(),new ArrayList<>()).size());
        });
        return ResultUtils.success(teamList);
    }

    //todo 查询分页 展示队伍已经加入的人数
